...
const ColorPickerNodeWrapper = styled.div`
  ...
  border-radius: 4px;
  box-shadow: ${(p) => (p.selected ? `0 0 0 0.25px ${p.theme.colors.violet}` : 'none')};
...
const ColorPickerNode = memo(({ data, id, selected }) => {
  ...
  return (
    <ColorPickerNodeWrapper color={data.color} value={data.value} selected={selected}>
      <div>{colorName} amount</div>
      <input type="range" min="0" max="255" value={data.value} onChange={onChange} className="nodrag" />
