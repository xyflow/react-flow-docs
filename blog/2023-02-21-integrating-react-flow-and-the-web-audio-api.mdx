...
a `get` function and returns an object with our initial state and the actions we
can use to update that state. Updates happen immutably and we can use the `set`
function for that. The `get` function is how we read the current state. And that's
it for zustand.
...
<CodeViewer
  ...
  options={{ editorHeight: '50vh' }}
  additionalFiles={['index.css', 'store.js', 'nodes/Osc.js']}
  dependencies={{ nanoid: 'latest', zustand: '4.3.6' }}
  showEditor={false}
/>
...
<CodeViewer
  ...
  options={{ editorHeight: '50vh' }}
  additionalFiles={['index.css', 'store.js', 'nodes/Osc.js', 'nodes/Out.js', 'nodes/Amp.js']}
  dependencies={{ nanoid: 'latest', zustand: '4.3.6' }}
  showEditor={false}
/>
...
With a lick of paint we have something that looks quite fancy.

[Rerender the demo flow showcasing a fully functional audio graph editor]
...
You can use the completed [source code](...) as a starting point, or you can just
keep building on top of what we've made today. We'd love to see what you build
so please share it with us over on our [Discord server](https://discord.com/invite/RVmnytFmGW)
