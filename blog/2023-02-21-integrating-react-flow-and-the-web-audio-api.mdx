...
//Generate a variable that will control the amplitude of the sin wave
const amp = ctx.createGain();

//multiply the value of the oscillator and the amplitude by connecting the two nodes
osc.connect(amp);
...
We only need three additional dependencies for this project: `reactflow` for our UI, `zustand` as our simple
state management library (that's what we use under the hood at React Flow) and
`nanoid` as a lightweight id generator.

```bash
npm install zustand nanoid reactflow
```
