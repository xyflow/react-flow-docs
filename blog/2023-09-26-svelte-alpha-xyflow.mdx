...
- New `nodeDragThreshold` prop to delay node drag events
- Improved performance for larger graphs with minimap
- Better key defaults and scroll behavior for Windows users
- Better default edge routing and label positions
- Compatibility with Astro, Remix, and older setups like webpack4 and React 17

And so, so many bugs have been squashed. Old bugs, new bugs. We feel like 6 year olds who just found an ant hill. For a compresnsive list, here's our [changelog](https://github.com/wbkd/react-flow/blob/main/packages/reactflow/CHANGELOG.md).
