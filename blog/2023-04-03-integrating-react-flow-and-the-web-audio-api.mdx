...
here and there, but for the most part [@todo remove one "for the most part"?] if you're new to React Flow you should be
...
we'll make a simple mouse theremin [@todo maybe add a link here? I didn't know what this is]. We'll use React for these examples and everything
...
UI (duh) [@todo what does "duh" mean? :D], `zustand` as our simple state management library (that's what we use
...
don't have re-renders when state we don't care about changes. [@todo is shallow necessary here?]
...
the volume, and an output node and a button to toggle DSP [@todo maybe explain DSP?] on and off.
  ...
  updateNode(id, data) {
    set({
      nodes: get().nodes.map(node =>
        node.id === id
          ? { ...node, data: Object.assign(node.data, data) } // @todo replace with spread?
          : node
      )
    })
  },
