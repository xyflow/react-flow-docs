...
Vite will scaffold out a simple React application for us. We can delete
...
Nothing happens in this example as-is. That's because an
...
Now let's take a look at how to build a React Flow project from scratch.

:::info
If you're already familiar with React Flow, you can comfortably skip ahead to [making some sounds](#do-sound-to-it).
:::

## Scaffolding a React Flow project

Once we've set up our React Flow project, we'll combine it with what we've learned about the Web Audio API, oscillators,
and gain nodes to build interactive audio-processing graphs. But first, we need to put together an empty React Flow app.

We already have a React app set up with Vite, so we'll keep using that. We only need three additional dependencies for this project: `reactflow` for our
...
So what's this weird `selector` thing all about? Zustand let's
...
Implementing a gain node is pretty much the same process, so we'll leave that one to you. Instead, we'll turn our attention to the output node. This node will
